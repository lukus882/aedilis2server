...
namespace Server.Items
{
    ...
    public class KenoBoard : Item
    {
        ...
        private ulong[] m_HitStats = new ulong[21];
#if PROFILE
        ...
        private TimeSpan m_IdleTimeout = TimeSpan.FromMinutes(5); // How long can a person be standing at the machine not playing?
        ...
        public bool Active
        {
            ...
            set
            {
                if (!m_Active && value)
                {
                    ...
                    this.PublicOverheadMessage(0, (this.Hue == 907 ? 0 : this.Hue), false, "CEOKeno online!");
                    string text = String.Format("{0} online!", this.Name);
                    MessagePlayers(text);
                }
                else if (m_Active && !value)
                {
                    m_OrigHue = this.Hue;
                    this.Hue = 1001;
                    this.PublicOverheadMessage(0, this.Hue, false, "CEOKeno offline.");
                    string text = String.Format("{0} offline.", this.Name);
                    MessagePlayers(text);
                }
                m_Active = value;
        ...
        public ThemeType Theme
        {
            ...
            set
            {
                if (m_Theme != value)
                {
                    ...
                    Active = false;
                    SetupTheme(value);
                    string text = String.Format("Theme Change: {0}", this.Name);
        ...
        public void DoWinSound(Mobile from, bool female, int amount)
        {
            ...
            if (amount > 499999)
            {
                for (int i = 0; i < 5; i++)
                    DoFireworks(from);
                from.PlaySound(female == true ? 824 : 1098);
            }
            else if (amount > 249999)
            {
                DoFireworks(from);
                from.PlaySound(female == true ? 823 : 1097);
            }
            else if (amount > 149999)
            {
                DoFireworks(from);
                from.PlaySound(female == true ? 783 : 1054);
            }
            else if (amount > 999)
                from.PlaySound(female == true ? 794 : 1066);
            else if (amount > 50)
            {
                switch (Utility.Random(7))
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        from.PlaySound(female == true ? 797 : 1069);
                        break;
                    case 2:
                        from.PlaySound(female == true ? 783 : 1054);
                        break;
        ...
        public bool CashCheck(Mobile m, PlayerInfo player, out int amount)
        {
            ...
            if (m == null || m.Backpack == null || player == null)
                return false;
ArrayList packlist = new ArrayList(m.Backpack.Items);
        ...
        public void RemoveIdlePlayers()
        {
            ...
            for (int i = 0; i < PlayerList.Count; i++)
            {
                ...
                if (player != null && !player.mobile.Deleted && DateTime.Now - player.LastPlayed > m_IdleTimeout)
                {
                    ...
                    player.mobile.SendMessage("You quit playing {0}.", this.Name);
                }
            }
            if (m_TotalPlayers != 0)
        ...
        private void InvalidatePlayers(string text)
        {
            ...
            for (int i = 0; i < PlayerList.Count; i++)
            {
                ...
                if (player.OnCredit < 0)
                {
                    Console.WriteLine("{0} has an invalidate credit amount for {1}({2}). Set to 0.", this.Name, player.mobile.Name, player.OnCredit);
                    player.OnCredit = 0;
                }
                if (player.OnCredit != 0)
...
ArrayList packlist = new ArrayList(m.Backpack.Items);
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage("You must be carrying your Captain's Cabin Membership Card to play {0}.", this.Name);
                return;
            }
            if (!from.Alive)
            {
            ...
            if (!from.InRange(this.GetWorldLocation(), 9) || !from.InLOS(this))
            {
                from.SendMessage("You are too far away from {0} or it is blocked from your location.", this.Name);
                return;
            }
            if (from.Hidden && from.AccessLevel == AccessLevel.Player) // Don't let someone sit at the KenoBoard and play hidden
            {
                from.Hidden = false;
                from.SendMessage("Playing {0} reveals you!", this.Name);
            }
