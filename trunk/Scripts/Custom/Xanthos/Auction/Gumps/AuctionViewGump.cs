	...
	public class AuctionViewGump : Gump
	{
		...
		private void MakeGump()
		{
			...
			if ( item.Item != null && 0 != itemHue )
			{
				...
				AddLabel( 37, 142, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 82 ]  );
			...
			// Reserve and bids
			AddLabel( 10, 175, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 68 ] );
			AddLabel( 45, 190, Xanthos.Utilities.Misc.kGreenHue, m_Auction.MinBid.ToString( "#,0" ) );

			AddLabel( 10, 280, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 69 ] );
			AddLabel( 45, 295, m_Auction.ReserveMet ? Xanthos.Utilities.Misc.kGreenHue : Xanthos.Utilities.Misc.kRedHue, m_Auction.ReserveMet ? "Met" : "Not Met" );

			AddLabel( 10, 210, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 70 ] );

			if ( m_Auction.HasBids )
				AddLabel( 45, 225, m_Auction.ReserveMet ? Xanthos.Utilities.Misc.kGreenHue : Xanthos.Utilities.Misc.kRedHue, m_Auction.HighestBid.Amount.ToString("#,0" ));
			else
				AddLabel( 45, 225, Xanthos.Utilities.Misc.kRedHue, AuctionSystem.ST[ 71 ]  );

			// Time remaining
			string timeleft = null;

			AddLabel( 10, 245, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 56 ] );
			...
			AddLabel( 45, 260, Xanthos.Utilities.Misc.kGreenHue, timeleft );
			
			// Bidding
			if ( m_Auction.CanBid( m_User ) && ! m_Auction.Expired )
			{
				AddLabel( 10, 318, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 79 ] );
				AddImageTiled( 9, 338, 112, 22, kBeigeBorderOuter );
				AddImageTiled( 10, 339, 110, 20, kBeigeBorderInner );
				AddAlphaRegion( 10, 339, 110, 20 );
				
				// Bid text: 0
				AddTextEntry( 10, 339, 110, 20, Xanthos.Utilities.Misc.kGreenHue, 0, @"" );
			...
			{
				// View bids: button 5
				AddLabel( 10, 338, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 80 ]  );
			...
			{
				AddLabel( 170, 10, Xanthos.Utilities.Misc.kGreenHue, string.Format( AuctionSystem.ST[ 231 ] , m_Auction.ItemName ));

				AddImageTiled( 169, 29, 317, 27, kBeigeBorderOuter );
				AddImageTiled( 170, 30, 315, 25, kBeigeBorderInner );
				AddAlphaRegion( 170, 30, 315, 25 );
				AddLabel( 185, 35, Xanthos.Utilities.Misc.kGreenHue, string.Format( AuctionSystem.ST[ 67 ] , m_Page + 1, m_Auction.ItemCount ) );
			...
			{
				AddLabel( 170, 10, Xanthos.Utilities.Misc.kGreenHue, m_Auction.ItemName );
			...
			AddLabel( 170, 175, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 81 ] );
			...
			{
				AddLabel( 350, 175, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 72 ] );
			...
			if ( m_Auction.AllowBuyNow && m_Auction.CanBid( m_User ) && !m_Auction.Expired )
			{
				...
				AddLabel( 205, 312, Xanthos.Utilities.Misc.kGreenHue, string.Format( AuctionSystem.ST[ 210 ], m_Auction.BuyNow.ToString( "#,0" )));
			}
			...
			if ( m_User.AccessLevel >= AuctionConfig.AuctionAdminAcessLevel )
			{
				...
				AddLabel( 205, 340, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 227 ] );
			}

			// Close button: 0
			AddButton( 455, 338, 4017, 4018, 0, GumpButtonType.Reply, 0 );
			AddLabel( 415, 340, Xanthos.Utilities.Misc.kLabelHue, AuctionSystem.ST[ 7 ] );
		}
