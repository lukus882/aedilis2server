        ...
        private static void PickerCallback(Mobile from, Map map, Point3D start, Point3D end, object state)
        {
            ...
            foreach (Point2D p in tiles.Keys)
            {
                ...
                foreach (Tile t in list)
                {
                    ...
                    if (generateTest)
                        AddIdentifyAddOnComponent(IdentifyAddon, id, xOffset, yOffset, zOffset, 0, -1, string.Format("({0}):{1},{2},{3}", m_TotalComponents, xOffset, yOffset, zOffset));
                }
            }
            // Statics & Items
            foreach (Item item in target)
            {
                ...
                if (((item.ItemData.Flags & TileFlag.LightSource) == TileFlag.LightSource) || (item.Hue != 0) || (item.Name != null)) // Use old method
                {
                    if (item.Name != null) // Have to do this one the old method
                    {
                        ...
                        if (generateTest)
                            AddIdentifyAddOnComponent(IdentifyAddon, id, xOffset, yOffset, zOffset, item.Hue, -1, string.Format("({0},{1}): {2}, {3}, {4}", m_TotalComponents, id, xOffset, yOffset, zOffset));
