...
namespace Server.Items
{
	...
	public class MoongateLibrary : Item
	{
		...
		[Constructable]
		public MoongateLibrary(Mobile owner, Point3D loc, bool active, ArrayList runebooks, bool decays, TimeSpan decaytime, TimeSpan timeleft, int cost, long amountcollected, AccessLevel accesslevel)
			: base(0xF6C)
		{
			Movable = false;
		...
		public void ReDeed(Mobile m)
		{
			...
			try
			{
				m.AddToBackpack(new MoongateLibraryDeed(m_Active, m_RuneBooks, m_Decays, m_DecayTime, m_Decays ? m_DecayEnd - DateTime.Now : TimeSpan.FromSeconds(0), m_Cost, m_AmountCollected, m_AccessLevel));
			}
			catch
			{
	...
	public class MoongateLibraryGump : Gump
	{
			...
			: base(100, 100)
		{
			...
			AddHtml(280, 5, 210, 20, "<basefont size=2 color=#0000ff>CEO's Moongate Library V1.2<basefont>", false, false);
			...
			if (m_RuneBooks.Count == 0)
				return;
			if (m_MoongateLibrary.Active || myGate) // Only put the buttons on an active gate
			{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (!m_MoongateLibrary.Active && !myGate)
			{
				m_Mobile.SendMessage("This Moongate Library is currently inactive.");
				return;
			}
...
namespace Server.Items
{
	public class MoongateLibraryDeed : Item
	{
		...
		[Constructable]
		public MoongateLibraryDeed(bool active, ArrayList runebooks, bool decays, TimeSpan decaytimer, TimeSpan timeleft, int cost, long amountcollected, AccessLevel accesslevel)
			: base(0x14F0)
		{
			Hue = 58;
			Weight = 1.0;
		...
		{
			this.Delete();
			new MoongateLibrary(from, loc, m_Active, m_RuneBooks, m_Decays, m_DecayTime, m_TimeLeft, m_Cost, m_AmountCollected, m_AccessLevel);
		}
