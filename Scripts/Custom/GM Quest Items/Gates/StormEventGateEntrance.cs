...
namespace Server.Items
{
	...
	public class StormEventGateEntrance : Item
	{
		...
		public bool Active
		{
			...
			set
			{
				...
				if (value && m_RuneBooks != null && m_RuneBooks.Count > 0)
				{
					...
					this.PublicOverheadMessage(0, this.Hue, false, "Storm Event Gate Entrance Active!");
				}
				else
				{
					m_Active = false;
					Hue = 826;
					this.PublicOverheadMessage(0, this.Hue, false, "Storm Event Gate Entrance Inactive.");
				}
				UpdateDate();
			}
		}
		...
		[Constructable]
		public StormEventGateEntrance(Mobile owner, Point3D loc, bool active, ArrayList runebooks, bool decays, TimeSpan decaytime, TimeSpan timeleft, int cost, long amountcollected, AccessLevel accesslevel)
			: base(0xF6C)
		{
			Movable = false;
			Name = "Event Gate";
		...
		public StormEventGateEntrance(Serial serial)
			: base(serial)
		{
		}
		...
		public void ReDeed(Mobile m)
		{
			...
			try
			{
				m.AddToBackpack(new StormEventGateEntranceDeed(m_Active, m_RuneBooks, m_Decays, m_DecayTime, m_Decays ? m_DecayEnd - DateTime.Now : TimeSpan.FromSeconds(0), m_Cost, m_AmountCollected, m_AccessLevel));
			}
			catch
			{
		...
		public void AddRunebook(Mobile m)
		{
			if (m_RuneBooks != null && m_RuneBooks.Count > 95) // 96 Maximum Runebooks
			{
				...
				StormEventSendLibraryGump(m, 0, 0, false);
			}
			else
			{
		...
		private class InternalTarget : Target
		{
			...
			private StormEventGateEntrance m_StormEventGateEntrance;

			public InternalTarget(StormEventGateEntrance StormEventGateEntrance, ArrayList RuneBooks)
				: base(12, false, TargetFlags.None)
			{
				this.CheckLOS = true;
				this.AllowGround = false;
				this.Range = 15;
				m_StormEventGateEntrance = StormEventGateEntrance;
				m_RuneBooks = RuneBooks;
			}
			...
			protected override void OnTarget(Mobile from, object o)
			{
				...
				else
				{
					if (validateRunebook(from, (Runebook)o))
					{
						...
						if (m_RuneBooks.Count == 1 && m_StormEventGateEntrance.Active)
							m_StormEventGateEntrance.Hue = orunebook.Hue == 1121 ? 58 : orunebook.Hue;
						m_StormEventGateEntrance.UpdateDate();
					}

				}
				m_StormEventGateEntrance.StormEventSendLibraryGump(from, 0, 0, false);
			}
			...
			else
			{
				...
				if (m.AccessLevel == AccessLevel.Player && (!Active || m_RuneBooks == null || m_RuneBooks.Count < 1))
				{
					if (!Owner(m))
					{
						...
						return StormEventSendLibraryGump(m, 0, -1, true);
				}
				else
					return StormEventSendLibraryGump(m, 0, -1, true);
			}
		...
		public bool StormEventSendLibraryGump(Mobile m, int page, int selected, bool sound)
		{
			m.CloseGump(typeof(StormEventGateEntranceGump));
			m.SendGump(new StormEventGateEntranceGump(m, this, m_LastChanged, m_RuneBooks, page, selected));
			if (sound)
		...
		{
			private StormEventGateEntrance m_ml;

			public DecayTimer(StormEventGateEntrance ml, TimeSpan delay)
				: base(delay)
			{
	...
	public class StormEventGateEntranceGump : Gump
	{
		...
		private bool myGate;
		public StormEventGateEntranceGump(Mobile mobile, StormEventGateEntrance rgate, DateTime LastChanged, ArrayList RuneBooks, int page, int selected)
			: base(100, 100)
		{
			m_Mobile = mobile;
			m_StormEventGateEntrance = rgate;
			m_LastChanged = LastChanged;
			m_RuneBooks = RuneBooks;
			m_page = page;
			m_selected = selected;
			myGate = m_StormEventGateEntrance.Owner(m_Mobile);
			AddPage(0);
			...
			AddHtml(280, 5, 210, 20, "<basefont size=2 color=#0000ff>Storms Event Gate 1.2<basefont>", false, false);
			if (m_StormEventGateEntrance.Cost != 0)
			{
				if (myGate)
					AddHtml(120, 20, 210, 20, String.Format("<basefont size=4 color=#FFCC33>Cost: {0}gp<basefont>", m_StormEventGateEntrance.Cost), false, false);
				else
					AddHtml(165, 5, 210, 20, String.Format("<basefont size=4 color=#FFCC33>Cost: {0}gp<basefont>", m_StormEventGateEntrance.Cost), false, false);
			...
			if (myGate)
			{
				...
				if (m_StormEventGateEntrance.Active)
			...
			if (m_RuneBooks.Count == 0)
				return;
			if (m_StormEventGateEntrance.Active || myGate) // Only put the buttons on an active gate
			{
		...
		private bool AccessAllowed(string securitystring, Mobile m)
		{
			...
			if (Insensitive.Contains(securitystring, "REDSONLY") && (m.Kills < 5))
				return false;
			if (Insensitive.Contains(securitystring, "OWNERONLY") && (m.Account != m_StormEventGateEntrance.m_Owner.Account))
				return false;
			...
			if (Insensitive.Contains(securitystring, "FEMALEONLY") && (!m.Female))
				return false;
			if (Insensitive.Contains(securitystring, "MYGUILD") && (m.GuildFealty != m_StormEventGateEntrance.m_Owner.GuildFealty))
				return false;
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			int index;
			Runebook runebook;
			if (info == null || info.ButtonID == 0 || m_Mobile == null || m_StormEventGateEntrance == null) // Cancel
				return;
			else if (m_Mobile.Deleted || m_StormEventGateEntrance.Deleted || m_Mobile.Map == null)
					return;
			else if (info.ButtonID == 2 && (myGate)) // Add Runebook
			{
				m_StormEventGateEntrance.AddRunebook(m_Mobile);
				return;
			}
			else if (info.ButtonID == 3 || info.ButtonID == 4 && (myGate))
			{
				m_StormEventGateEntrance.Active = !m_StormEventGateEntrance.Active;
				m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, 0, 0, false);
				return;
			}
			else if (info.ButtonID == 5 && (myGate))
			{
				m_Mobile.CloseGump(typeof(StormEventGateConfirmDeleteGump));
				m_Mobile.SendGump(new StormEventGateConfirmDeleteGump(true, m_Mobile, m_StormEventGateEntrance, null, 0));
				return;
			}
			else if (info.ButtonID == 6 && (myGate))
			{
				m_StormEventGateEntrance.ReDeed(m_Mobile);
				return;
			}
			else if (info.ButtonID == 7 && (myGate))
			{
				m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, 0, 0, false);
				m_Mobile.CloseGump(typeof(StormEventGateEntranceHelpGump));
				m_Mobile.SendGump(new StormEventGateEntranceHelpGump());
				return;
			}
			else if (info.ButtonID >= 50 && info.ButtonID < 60)
			{
				m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, info.ButtonID - 50, 0, false);
				return;
			}
			else if (info.ButtonID >= 100 && info.ButtonID < 120)
			{
				m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, m_page, info.ButtonID - 100, false);
				return;
			}
			else if ((info.ButtonID >= 200 && info.ButtonID < 220) && (myGate))
			{
				...
				if (index < 0 || index >= m_RuneBooks.Count || m_RuneBooks[index] == null)
				{
					...
					m_StormEventGateEntrance.RemoveDeleted();
					m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, 0, 0, true);
					return;
				}
				runebook = ((Runebook)m_RuneBooks[index]);
				m_Mobile.SendGump(new StormEventGateConfirmDeleteGump(false, m_Mobile, m_StormEventGateEntrance, runebook.Description, index));
				return;
			}
			else if ((info.ButtonID >= 300 && info.ButtonID < 320) && (myGate))
			{
				...
				if (index <= 0) // should never happen unless there's razor/button spoofing going on.
					return;
				m_StormEventGateEntrance.ShiftUp(m_page * 12 + info.ButtonID - 300);
				m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, m_page, (info.ButtonID - 300) == 0 ? 0 : info.ButtonID - 301, false);
				return;
			}
			else if ((info.ButtonID >= 400 && info.ButtonID < 420) && (myGate))
			{
				...
				m_StormEventGateEntrance.ShiftDown(m_page * 12 + info.ButtonID - 400);
				m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, m_page, nbuttonloc, false);
				return;
			}
			if (m_StormEventGateEntrance.LastChanged != m_LastChanged)
			{
				m_Mobile.SendMessage("Event Gate has changed, choose again.");
				m_StormEventGateEntrance.RemoveDeleted();
				m_StormEventGateEntrance.StormEventSendLibraryGump(m_Mobile, 0, 0, true);
			...
			if (!m_StormEventGateEntrance.Active && !myGate)
			{
			...
			if (!m_Mobile.InRange(m_StormEventGateEntrance.GetWorldLocation(), 1) || m_Mobile.Map != m_StormEventGateEntrance.Map)
			{
				m_Mobile.SendLocalizedMessage(1019002); // You are too far away to use the gate.
			}
			else if (m_Mobile.Player && m_Mobile.Kills >= 5 && entry.Map != Map.Felucca && !m_StormEventGateEntrance.m_RedsAnywhere)
			{
			...
			{
				m_Mobile.CloseGump(typeof(StormEventGateEntranceConfirmGump));
				m_Mobile.SendGump(new StormEventGateEntranceConfirmGump(m_StormEventGateEntrance, m_Mobile, entry.Location, entry.Map, entry.Description, 2));

			}
			else if (m_Mobile.AccessLevel == AccessLevel.Player && entry.Map == Map.Felucca && m_Mobile.Map != Map.Felucca)
			{
				m_Mobile.CloseGump(typeof(StormEventGateEntranceConfirmGump));
				m_Mobile.SendGump(new StormEventGateEntranceConfirmGump(m_StormEventGateEntrance, m_Mobile, entry.Location, entry.Map, entry.Description, 0));

			}
			else if (m_Mobile.AccessLevel == AccessLevel.Player && Insensitive.Contains(entry.Description, "warnplayer"))
			{
				m_Mobile.CloseGump(typeof(StormEventGateEntranceConfirmGump));
				m_Mobile.SendGump(new StormEventGateEntranceConfirmGump(m_StormEventGateEntrance, m_Mobile, entry.Location, entry.Map, entry.Description, 1));
			...
			{
				if (m_StormEventGateEntrance.Cost == 0 || myGate || CollectGold(m_Mobile, m_StormEventGateEntrance.Cost) )
				{
					m_StormEventGateEntrance.AddToCollectedAmount(m_StormEventGateEntrance.Cost);
...
//--<Banks Items In Backpack>----------------------------------<End>
	...
	public class StormEventGateEntranceConfirmGump : Gump
	{
		private StormEventGateEntrance m_StormEventGateEntrance;
		private Mobile m_Mobile;
		...
		public StormEventGateEntranceConfirmGump(StormEventGateEntrance StormEventGateEntrance, Mobile mobile, Point3D location, Map map, string runedesc, int message)
			: base(10, 10)
		{
			m_Mobile = mobile;
			m_location = location;
	...
	public class StormEventGateConfirmDeleteGump : Gump
	{
		private bool m_all;
		private Mobile m_From;
		private StormEventGateEntrance m_Gate;
		private string m_rdesc;
		private int m_index;

		public StormEventGateConfirmDeleteGump(bool all, Mobile from, StormEventGateEntrance gate, string rdesc, int index)
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			m_Gate.StormEventSendLibraryGump(m_From, 0, 0, false);
		}
	}

	public class StormEventGateEntranceHelpGump : Gump
	{
		public StormEventGateEntranceHelpGump()
			: base(Core.AOS ? 110 : 20, Core.AOS ? 100 : 30)
		{
...
namespace Server.Items
{
	public class StormEventGateEntranceDeed : Item
	{
		private bool m_Active = false;
		private AccessLevel m_AccessLevel = AccessLevel.Player;
		private ArrayList m_RuneBooks = new ArrayList();
		...
		[Constructable]
		public StormEventGateEntranceDeed(bool active, ArrayList runebooks, bool decays, TimeSpan decaytimer, TimeSpan timeleft, int cost, long amountcollected, AccessLevel accesslevel)
			: base(0x14F0)
		{
			Hue = 58;
			Weight = 1.0;
		...
		[Constructable]
		public StormEventGateEntranceDeed()
			: base(0x14F0)
		{
			Hue = 58;
			Weight = 1.0;
		...
		public StormEventGateEntranceDeed(Serial serial)
			: base(serial)
		{
		}
		...
		{
			this.Delete();
			new StormEventGateEntrance(from, loc, m_Active, m_RuneBooks, m_Decays, m_DecayTime, m_TimeLeft, m_Cost, m_AmountCollected, m_AccessLevel);
		}
