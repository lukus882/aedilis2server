...
namespace Server.Items
{
	...
	public class CoachEventGateEntrance : Item
	{
		...
		public bool Active
		{
			...
			set
			{
				...
				if (value && m_RuneBooks != null && m_RuneBooks.Count > 0)
				{
					...
					this.PublicOverheadMessage(0, this.Hue, false, "Coach Event Gate Entrance Active!");
				}
				else
				{
					m_Active = false;
					Hue = 826;
					this.PublicOverheadMessage(0, this.Hue, false, "Coach Event Gate Entrance Inactive.");
				}
				UpdateDate();
			}
		}
		...
		[Constructable]
		public CoachEventGateEntrance(Mobile owner, Point3D loc, bool active, ArrayList runebooks, bool decays, TimeSpan decaytime, TimeSpan timeleft, int cost, long amountcollected, AccessLevel accesslevel)
			: base(0xF6C)
		{
			Movable = false;
			Name = "Event Gate";
			...
			: base(0xF6C)
		{
			...
			m_Decays = false;
			m_DecayRunning = false;
			m_DecayEnd = DateTime.Now + m_DecayTime;
			Effects.PlaySound(this.Location, this.Map, 0x20E);
			UpdateDate();
		}

		public CoachEventGateEntrance(Serial serial)
			: base(serial)
		{
		}
		...
		public void ReDeed(Mobile m)
		{
			...
			try
			{
				m.AddToBackpack(new CoachEventGateEntranceDeed(m_Active, m_RuneBooks, m_Decays, m_DecayTime, m_Decays ? m_DecayEnd - DateTime.Now : TimeSpan.FromSeconds(0), m_Cost, m_AmountCollected, m_AccessLevel));
			}
			catch
			{
				m.SendMessage("There's a problem Re-Deeding the Event Gate. Page for help.");
				return;
			}
		...
		public void AddRunebook(Mobile m)
		{
			...
			{
				m.SendMessage("No more runebooks may be added to this Event Gate!");
				CoachEventSendLibraryGump(m, 0, 0, false);
			}
			else
			{
		...
		private class InternalTarget : Target
		{
			...
			private CoachEventGateEntrance m_CoachEventGateEntrance;

			public InternalTarget(CoachEventGateEntrance CoachEventGateEntrance, ArrayList RuneBooks)
				: base(12, false, TargetFlags.None)
			{
				this.CheckLOS = true;
				this.AllowGround = false;
				this.Range = 15;
				m_CoachEventGateEntrance = CoachEventGateEntrance;
				m_RuneBooks = RuneBooks;
			}
			...
			protected override void OnTarget(Mobile from, object o)
			{
				...
				else
				{
					if (validateRunebook(from, (Runebook)o))
					{
						...
						if (m_RuneBooks.Count == 1 && m_CoachEventGateEntrance.Active)
							m_CoachEventGateEntrance.Hue = orunebook.Hue == 1121 ? 58 : orunebook.Hue;
						m_CoachEventGateEntrance.UpdateDate();
					}

				}
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(from, 0, 0, false);
			}
		...
		public bool UseGate(Mobile m)
		{
			...
			else
			{
				...
				if (m.AccessLevel == AccessLevel.Player && (!Active || m_RuneBooks == null || m_RuneBooks.Count < 1))
				{
					if (!Owner(m))
					{
						m.SendMessage("This Event Gate is currently inactive or has no locations.");
						return false;
					}
					else
						return CoachEventSendLibraryGump(m, 0, -1, true);
				}
				else
					return CoachEventSendLibraryGump(m, 0, -1, true);
			}
		...
		public bool CoachEventSendLibraryGump(Mobile m, int page, int selected, bool sound)
		{
			m.CloseGump(typeof(CoachEventGateEntranceGump));
			m.SendGump(new CoachEventGateEntranceGump(m, this, m_LastChanged, m_RuneBooks, page, selected));
			if (sound)
		...
		{
			private CoachEventGateEntrance m_ml;

			public DecayTimer(CoachEventGateEntrance ml, TimeSpan delay)
				: base(delay)
			{
	...
	public class CoachEventGateEntranceGump : Gump
	{
		...
		private bool myGate;
		public CoachEventGateEntranceGump(Mobile mobile, CoachEventGateEntrance rgate, DateTime LastChanged, ArrayList RuneBooks, int page, int selected)
			: base(100, 100)
		{
			m_Mobile = mobile;
			m_CoachEventGateEntrance = rgate;
			m_LastChanged = LastChanged;
			m_RuneBooks = RuneBooks;
			m_page = page;
			m_selected = selected;
			myGate = m_CoachEventGateEntrance.Owner(m_Mobile);
			AddPage(0);
			...
			AddHtml(280, 5, 210, 20, "<basefont size=2 color=#0000ff>Coachs Event Gate 1.2<basefont>", false, false);
			if (m_CoachEventGateEntrance.Cost != 0)
			{
				if (myGate)
					AddHtml(120, 20, 210, 20, String.Format("<basefont size=4 color=#FFCC33>Cost: {0}gp<basefont>", m_CoachEventGateEntrance.Cost), false, false);
				else
					AddHtml(165, 5, 210, 20, String.Format("<basefont size=4 color=#FFCC33>Cost: {0}gp<basefont>", m_CoachEventGateEntrance.Cost), false, false);
			...
			if (myGate)
			{
				...
				if (m_CoachEventGateEntrance.Active)
			...
			if (m_RuneBooks.Count == 0)
				return;
			if (m_CoachEventGateEntrance.Active || myGate) // Only put the buttons on an active gate
			{
		...
		private bool AccessAllowed(string securitystring, Mobile m)
		{
			...
			if (Insensitive.Contains(securitystring, "REDSONLY") && (m.Kills < 5))
				return false;
			if (Insensitive.Contains(securitystring, "OWNERONLY") && (m.Account != m_CoachEventGateEntrance.m_Owner.Account))
				return false;
			...
			if (Insensitive.Contains(securitystring, "FEMALEONLY") && (!m.Female))
				return false;
			if (Insensitive.Contains(securitystring, "MYGUILD") && (m.GuildFealty != m_CoachEventGateEntrance.m_Owner.GuildFealty))
				return false;
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			int index;
			Runebook runebook;
			if (info == null || info.ButtonID == 0 || m_Mobile == null || m_CoachEventGateEntrance == null) // Cancel
				return;
			else if (m_Mobile.Deleted || m_CoachEventGateEntrance.Deleted || m_Mobile.Map == null)
					return;
			else if (info.ButtonID == 2 && (myGate)) // Add Runebook
			{
				m_CoachEventGateEntrance.AddRunebook(m_Mobile);
				return;
			}
			else if (info.ButtonID == 3 || info.ButtonID == 4 && (myGate))
			{
				m_CoachEventGateEntrance.Active = !m_CoachEventGateEntrance.Active;
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, 0, 0, false);
				return;
			}
			else if (info.ButtonID == 5 && (myGate))
			{
				m_Mobile.CloseGump(typeof(CoachEventGateConfirmDeleteGump));
				m_Mobile.SendGump(new CoachEventGateConfirmDeleteGump(true, m_Mobile, m_CoachEventGateEntrance, null, 0));
				return;
			}
			else if (info.ButtonID == 6 && (myGate))
			{
				m_CoachEventGateEntrance.ReDeed(m_Mobile);
				return;
			}
			else if (info.ButtonID == 7 && (myGate))
			{
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, 0, 0, false);
				m_Mobile.CloseGump(typeof(CoachEventGateEntranceHelpGump));
				m_Mobile.SendGump(new CoachEventGateEntranceHelpGump());
				return;
			}
			else if (info.ButtonID >= 50 && info.ButtonID < 60)
			{
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, info.ButtonID - 50, 0, false);
				return;
			}
			else if (info.ButtonID >= 100 && info.ButtonID < 120)
			{
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, m_page, info.ButtonID - 100, false);
				return;
			}
			else if ((info.ButtonID >= 200 && info.ButtonID < 220) && (myGate))
			{
				...
				{
					m_Mobile.SendMessage("Event Gate has changed, choose again.");
					m_CoachEventGateEntrance.RemoveDeleted();
					m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, 0, 0, true);
					return;
				}
				runebook = ((Runebook)m_RuneBooks[index]);
				m_Mobile.SendGump(new CoachEventGateConfirmDeleteGump(false, m_Mobile, m_CoachEventGateEntrance, runebook.Description, index));
				return;
			}
			else if ((info.ButtonID >= 300 && info.ButtonID < 320) && (myGate))
			{
				...
				if (index <= 0) // should never happen unless there's razor/button spoofing going on.
					return;
				m_CoachEventGateEntrance.ShiftUp(m_page * 12 + info.ButtonID - 300);
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, m_page, (info.ButtonID - 300) == 0 ? 0 : info.ButtonID - 301, false);
				return;
			}
			else if ((info.ButtonID >= 400 && info.ButtonID < 420) && (myGate))
			{
				...
				m_CoachEventGateEntrance.ShiftDown(m_page * 12 + info.ButtonID - 400);
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, m_page, nbuttonloc, false);
				return;
			}
			if (m_CoachEventGateEntrance.LastChanged != m_LastChanged)
			{
				m_Mobile.SendMessage("Event Gate has changed, choose again.");
				m_CoachEventGateEntrance.RemoveDeleted();
				m_CoachEventGateEntrance.CoachEventSendLibraryGump(m_Mobile, 0, 0, true);
			...
			if (!m_CoachEventGateEntrance.Active && !myGate)
			{
				m_Mobile.SendMessage("This Event Gate is currently inactive.");
				return;
			}
			...
			//Console.WriteLine("Mobile: {0} RuneBook: {1} Sel: {2} Desc: {3}", m_Mobile, runebook, selected, runebook.Description);
			if (runebook.Description != null)
			{
				if ((m_Mobile.AccessLevel == AccessLevel.Player) && (runebook.Description.Equals("SKIP") || runebook.Description.Equals("BREAK") || Insensitive.StartsWith(runebook.Description, "TITLE,")))
				{
			...
			if (runebook.Description != null)
			{
				keywordargs = ParseString(runebook.Description, 15, ":");
			...
			if (!m_Mobile.InRange(m_CoachEventGateEntrance.GetWorldLocation(), 1) || m_Mobile.Map != m_CoachEventGateEntrance.Map)
			{
				m_Mobile.SendLocalizedMessage(1019002); // You are too far away to use the gate.
			}
			else if (m_Mobile.Player && m_Mobile.Kills >= 5 && entry.Map != Map.Felucca && !m_CoachEventGateEntrance.m_RedsAnywhere)
			{
			...
			{
				m_Mobile.CloseGump(typeof(CoachEventGateEntranceConfirmGump));
				m_Mobile.SendGump(new CoachEventGateEntranceConfirmGump(m_CoachEventGateEntrance, m_Mobile, entry.Location, entry.Map, entry.Description, 2));

			}
			else if (m_Mobile.AccessLevel == AccessLevel.Player && entry.Map == Map.Felucca && m_Mobile.Map != Map.Felucca)
			{
				m_Mobile.CloseGump(typeof(CoachEventGateEntranceConfirmGump));
				m_Mobile.SendGump(new CoachEventGateEntranceConfirmGump(m_CoachEventGateEntrance, m_Mobile, entry.Location, entry.Map, entry.Description, 0));

			}
			else if (m_Mobile.AccessLevel == AccessLevel.Player && Insensitive.Contains(entry.Description, "warnplayer"))
			{
				m_Mobile.CloseGump(typeof(CoachEventGateEntranceConfirmGump));
				m_Mobile.SendGump(new CoachEventGateEntranceConfirmGump(m_CoachEventGateEntrance, m_Mobile, entry.Location, entry.Map, entry.Description, 1));

			}
			else if ( m_Mobile != null && m_Mobile.Player && m_Mobile.Mounted && m_Mobile.Followers != 0 || m_Mobile is BaseCreature )
			{
				m_Mobile.SendMessage("You may NOT enter while mounted."); // You may not enter while mounted.

			}
			else if ( m_Mobile != null && m_Mobile.Player && m_Mobile.Followers != 0 )
			{	
				m_Mobile.SendMessage("You may NOT enter with followers."); // You may not with followers
			}
			else
			{
				if (m_CoachEventGateEntrance.Cost == 0 || myGate || CollectGold(m_Mobile, m_CoachEventGateEntrance.Cost) )
				{
					m_CoachEventGateEntrance.AddToCollectedAmount(m_CoachEventGateEntrance.Cost);
...
//--<Banks Items In Backpack>----------------------------------<Start>
				...
				ArrayList equipitems = new ArrayList(m_Mobile.Items);
				...
				Container pouch = m_Mobile.Backpack;
...
//--<Banks Items In Backpack>----------------------------------<End>
					...
					m_Mobile.AddToBackpack( new CoachsCrazyQuestRobe() );
				}
				else
				{
	...
	public class CoachEventGateEntranceConfirmGump : Gump
	{
		private CoachEventGateEntrance m_CoachEventGateEntrance;
		private Mobile m_Mobile;
		...
		public CoachEventGateEntranceConfirmGump(CoachEventGateEntrance CoachEventGateEntrance, Mobile mobile, Point3D location, Map map, string runedesc, int message)
			: base(10, 10)
		{
			m_Mobile = mobile;
			m_location = location;
	...
	public class CoachEventGateConfirmDeleteGump : Gump
	{
		private bool m_all;
		private Mobile m_From;
		private CoachEventGateEntrance m_Gate;
		private string m_rdesc;
		private int m_index;

		public CoachEventGateConfirmDeleteGump(bool all, Mobile from, CoachEventGateEntrance gate, string rdesc, int index)
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			m_Gate.CoachEventSendLibraryGump(m_From, 0, 0, false);
		}
	}

	public class CoachEventGateEntranceHelpGump : Gump
	{
		public CoachEventGateEntranceHelpGump()
			: base(Core.AOS ? 110 : 20, Core.AOS ? 100 : 30)
		{
			...
			AddPage(0);
			AddBackground(0, 0, width, height, 5054);
			string text = "<basefont color=#007fff><u><center>Event Gate Instructions</center></u>" +
			"<basefont color=white>This Event Gate allows you store up to 96 runebooks in one moongate, giving you the ability to have " +
			...
			"to do some simple formatting of your Event Gate gump.<br>" +
...
namespace Server.Items
{
	public class CoachEventGateEntranceDeed : Item
	{
		private bool m_Active = false;
		private AccessLevel m_AccessLevel = AccessLevel.Player;
		private ArrayList m_RuneBooks = new ArrayList();
		...
		private DateTime m_DecayEnd;
		private bool m_DecayRunning = false;
		private TimeSpan m_TimeLeft = TimeSpan.FromDays(90);
		...
		[Constructable]
		public CoachEventGateEntranceDeed(bool active, ArrayList runebooks, bool decays, TimeSpan decaytimer, TimeSpan timeleft, int cost, long amountcollected, AccessLevel accesslevel)
			: base(0x14F0)
		{
			Hue = 58;
			Weight = 1.0;
			Name = "Event Gate Deed";
		...
		[Constructable]
		public CoachEventGateEntranceDeed()
			: base(0x14F0)
		{
			Hue = 58;
			Weight = 1.0;
		...
		public CoachEventGateEntranceDeed(Serial serial)
			: base(serial)
		{
		}
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			writer.Write((int)m_AccessLevel);// Version 2
		...
		public bool ValidatePlacement(Mobile from, Point3D loc)
		{
			...
			{
				from.SendMessage("The Event Gate can only be placed in your house.");
				return false;
			}
		...
		{
			this.Delete();
			new CoachEventGateEntrance(from, loc, m_Active, m_RuneBooks, m_Decays, m_DecayTime, m_TimeLeft, m_Cost, m_AmountCollected, m_AccessLevel);
		}
