			...
			private class InternalTarget : Target
			{
				...
				protected override void OnTarget( Mobile from, object targ )
				{
					if ( targ is IPoint2D )
					{
						...
						if ( m_Creature is NightSheep )
						{
							double skill = from.Skills[SkillName.Herding].Value;
							
							if ( skill < 65.0 )
							{
								from.SendMessage( "You are not experienced enough to herd nightsheep!" );
								NightSheepSystem.CheckSander( false, m_Creature, from );
							}
							else
							{
								double chance = ( skill / 800 ) * 5;
								if ( chance > Utility.RandomDouble() ) 
								{
									m_Creature.Controlled = true;
									m_Creature.ControlMaster = from;
									m_Creature.ControlOrder = OrderType.Come;
									from.SendMessage( "You sucessfully herd the nightsheep." );
									from.CheckSkill( SkillName.Herding, 0.0, 120.0 );
									NightSheepSystem.CheckSander( true, m_Creature, from );
									NightSheepSystem.AddSheep( from, m_Creature );
								}
								else
								{
									from.SendMessage( "You fail to herd the nightsheep." );
									from.CheckSkill( SkillName.Herding, 0.0, 120.0 );
									NightSheepSystem.CheckSander( false, m_Creature, from );
								}
