...
namespace Knives.Chat3
{
    public class ChannelGump : GumpPlus
    {
        ...
        protected override void BuildGump()
        {
            ...
            AddTextField(width / 2 + 10, y, 70, 21, 0x480, 0, "" + c_Channel.Name);
            AddButton(width/2-5, y + 3, 0x2716, "Submit", new GumpCallback(Submit));
            ...
            AddImageTiled(width/2+10, y, 70, 21, 0xBBA);
            AddTextField(width/2+10, y, 70, 21, 0x480, 1, "");
        ...
        private void AutoNewChars()
        {
            ...
            if (c_Channel.NewChars)
                foreach (Data data in Data.Datas.Values)
                    if(!c_Channel.Mobiles.Contains(data.Mobile))
                        c_Channel.Mobiles.Add(data.Mobile);
        ...
        private void AddCommand()
        {
            ...
            if (c_Channel.Commands.Contains(GetTextField(1).ToLower()))
            {
                c_Channel.Commands.Remove(GetTextField(1).ToLower());
                Channel.RemoveCommand(GetTextField(1).ToLower());
            }
            else
            {
                c_Channel.Commands.Add(GetTextField(1).ToLower());
                Channel.AddCommand(GetTextField(1).ToLower());
            }
        ...
        private class ChannelSelectGump : GumpPlus
        {
            ...
            protected override void BuildGump()
            {
                ...
                AddHtml(0, y, 150, "<CENTER>" + General.Local(38));

                foreach (Channel c in Channel.Channels)
                {
                    AddHtml(50, y += 20, 100, c.NameFor(Owner));
                    AddButton(30, y + 3, 0x2716, "Select Channel", new GumpStateCallback(SelectChannel), c);
                }
