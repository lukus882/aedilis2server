...
namespace Knives.Chat3
{
	...
	public class IrcConnection
	{
		...
		{try{
            ...
            foreach (Data data in Data.Datas.Values)
                if (data.Mobile.HasGump(typeof(IrcGump)))
                    General.RefreshGump(data.Mobile, typeof(IrcGump));
		...
		{
            if (Channel.GetByName("IRC") == null)
                return;

            foreach (Mobile m in Channel.GetByName("IRC").Mobiles)
        ...
        public void Disconnect(bool reconn)
        {
            ...
            catch (Exception e) { Errors.Report(String.Format("IrcConnection-> Disconnect")); Console.WriteLine(e.Message); Console.WriteLine(e.Source); Console.WriteLine(e.StackTrace); }
        }
	}
}