...
{
  public class SuperDupe
  {
    public static void Initialize()
    {
      CommandSystem.Register( "SuperDupe", AccessLevel.GameMaster, new CommandEventHandler( SuperDupe_OnCommand ) );
      CommandSystem.Register( "SuperDupeInBag", AccessLevel.GameMaster, new CommandEventHandler( SuperDupeInBag_OnCommand ) );
    }

    [Usage( "SuperDupe [amount]" )]
    [Description( "Dupes a targeted item." )]
    private static void SuperDupe_OnCommand( CommandEventArgs e )
    {
      int amount = 1;
      if ( e.Length >= 1 )
        amount = e.GetInt32( 0 );
      e.Mobile.Target = new SuperDupeTarget( false, amount > 0 ? amount : 1 );
      e.Mobile.SendMessage( "What do you wish to Super Dupe?" );
    }

    [Usage( "SuperDupeInBag <count>" )]
    [Description( "Super Dupes an item at it's current location (count) number of times." )]
    private static void SuperDupeInBag_OnCommand( CommandEventArgs e )
    {
      int amount = 1;
      if ( e.Length >= 1 )
        amount = e.GetInt32( 0 );

      e.Mobile.Target = new SuperDupeTarget( true, amount > 0 ? amount : 1 );
      e.Mobile.SendMessage( "What do you wish to Super Dupe?" );
    }

    private class SuperDupeTarget : Target
    {
      private bool m_InBag;
      private int m_Amount;

      public SuperDupeTarget( bool inbag, int amount ) : base( 15, false, TargetFlags.None )
      {
        m_InBag = inbag;
        m_Amount = amount;
      }
      ...
      protected override void OnTarget( Mobile from, object targ )
      {
        ...
        if ( !(targ is Item) )
        {
          from.SendMessage( "You can only Super Dupe items." );
          return;
        }
        ...
        try 
        {
          ...
          {
            InternalSuperDupe( ( Item )targ, pack, from );
          }
        ...
        if ( !done )
        {
          from.SendMessage( "Unable to Super Dupe. Item must have a 0 parameter constructor." );
        }
      }
      ...
      private static void InternalSuperDupe ( Item src, Container pack, Mobile from )
      {
        ...
        foreach ( ConstructorInfo c in info )
        {
          ...
          if ( paramInfo.Length == 0 )
          {
            ...
            if ( o != null && o is Item )
            {
              ...
              if ( newItem is Container ) 
              {
                ...
                for ( int i = 0; i < items.Length; i++ ) 
                {
                  ...
                  InternalSuperDupe(items[i], ( Container )newItem, from );
                }
              }
            ...
            {
              from.SendMessage("Target is not a valid item anymore or vanished before the Super Dupe.");
            }
          }
        }
